...
public class VoxelMap
{
    public class Level
    {
        ...
        public bool InBounds((int x, int y, int z) v) => 
            InBounds(v.x, v.y, v.z);

        public (int x, int y, int z) ClampMin(int x, int y, int z) => 
            (Math.Max(x, 0), Math.Max(y, 0), Math.Max(z, 0));

        public (int x, int y, int z) ClampMin((int x, int y, int z) v) => 
            ClampMin(v.x, v.y, v.z);

        public (int x, int y, int z) ClampMax(int x, int y, int z) => 
            (Math.Min(x, NumCellsX - 1), Math.Min(y, NumCellsY - 1), Math.Min(z, NumCellsZ - 1));

        public (int x, int y, int z) ClampMax((int x, int y, int z) v) => 
            ClampMax(v.x, v.y, v.z);

        public (int x, int y, int z) Clamp(int x, int y, int z) => 
            ClampMax(ClampMin(x, y, z));

        public (int x, int y, int z) Clamp((int x, int y, int z) v) => 
        ...
        public ushort VoxelToIndex(int x, int y, int z) => 
            (ushort)((((z << ShiftXZ) + x) << ShiftYX) + y);

        public ushort VoxelToIndex((int x, int y, int z) v) => 
            VoxelToIndex(v.x, v.y, v.z);
    }

    public class Tile(VoxelMap owner, Vector3 boundsMin, Vector3 boundsMax, int level)
    {
            ...
            Contents
        {
            get;
            init;
        } = new ushort[owner.Levels[level].NumCellsTotal]; // high bit unset: empty voxel (TODO: region id in low bits?); high bit set: voxel with solid geometry (VoxelIdMask if leaf, subvoxel index otherwise); order is (y,x,z)

        public List<Tile> Subdivision { get; init; } = [];
        ...
        public Vector3 VoxelToWorld((int x, int y, int z) v) => 
        ...
        public (Vector3 min, Vector3 max) CalculateSubdivisionBounds((int x, int y, int z) v) => 
    ...
    public static ulong EncodeIndex(ushort tileIndex, ulong subIndex = InvalidVoxel) => 
        (subIndex << IndexLevelShift) + tileIndex;
    ...
    public (ulong voxel, bool empty) FindLeafVoxel(Vector3 p) => 
        RootTile.FindLeafVoxel(p);

    public ulong WorldToVoxel(Vector3 p)
    {
    ...
    public void Build(Voxelizer vox, int tx, int tz)
    {
        ...
        for (var ty = 0; ty < ny; ++ty, ++idx) BuildTile(RootTile, idx, 0, ty, 0, voxelizers);
    }
    ...
    private void BuildTile(Tile parent, ushort index, int x0, int y0, int z0, Span<Voxelizer> source)
    {
        ...
        else
        {
            ...
            var     xOff = x0 * l.NumCellsX;
            var     yOff = y0 * l.NumCellsY;
            var     zOff = z0 * l.NumCellsZ;
            ushort  i    = 0;
